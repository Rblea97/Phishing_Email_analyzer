...
class AIPhishingAnalyzer:
    ...
    def _create_analysis_prompt(self, parsed_email: ParsedEmail) -> str:
        ...
        subject = parsed_email.headers.subject[:200] if parsed_email.headers.subject else "No subject"
        sender = parsed_email.headers.from_addr[:100] if parsed_email.headers.from_addr else "Unknown sender"
        ...
        prompt = f"""You are a cybersecurity analyst. Analyze this email step-by-step for phishing indicators:
...
Evidence IDs should be uppercase with underscores (e.g., SPF_FAIL, SUSPICIOUS_URL, URGENT_LANGUAGE)."""
    ...
    def _truncate_prompt(self, prompt: str) -> str:
        ...
        logger.info(f"Prompt truncated from {len(prompt)} to {len(''.join(truncated_lines))} characters")
        ...
        for attempt in range(MAX_RETRIES + 1):
            try:
                response = self.client.chat.completions.create(
                    model=MODEL_NAME,
                    messages=[
                        {"role": "system", "content": "You are a cybersecurity expert specializing in phishing detection. Respond only with valid JSON."},
        ...
        try:
            ...
            if error or not response_data:
                return AIAnalysisResult(
                    score=50,
                    label="Suspicious",
                    evidence=[{"id": "AI_ERROR", "description": error or "Analysis failed", "weight": 10}],
                    tokens_used=0,
                    cost_estimate=0.0,
            ...
            if not is_valid:
                ...
                return AIAnalysisResult(
                    score=50,
                    label="Suspicious", 
                    evidence=[{"id": "VALIDATION_ERROR", "description": f"Response validation failed: {validation_error}", "weight": 10}],
            ...
            logger.info(f"AI analysis completed: score={sanitized_data['score']}, "
            ...
            return AIAnalysisResult(
                score=50,
                label="Suspicious",
                evidence=[{"id": "SYSTEM_ERROR", "description": f"System error: {str(e)}", "weight": 10}],
                tokens_used=0,
                cost_estimate=0.0,
                processing_time_ms=(time.time() - start_time) * 1000,
                success=False,
...
def analyze_email_with_ai(parsed_email: ParsedEmail) -> AIAnalysisResult:
    ...
    try:
        ...
        return AIAnalysisResult(
            score=50,
            label="Suspicious",
            evidence=[{"id": "CONFIG_ERROR", "description": "AI service not available", "weight": 10}],
            tokens_used=0,
            cost_estimate=0.0,
