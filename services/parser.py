...
class EmailParser:
    ...
    def parse_email(self, email_content: bytes, filename: str = "unknown") -> ParsedEmail:
        ...
        try:
            ...
            if parsed_size > MAX_PARSED_TEXT_SIZE:
                security_warnings.append(f"Parsed content truncated: {parsed_size} > {MAX_PARSED_TEXT_SIZE}")
    ...
    def _extract_headers(self, msg: email.message.EmailMessage, warnings: List[str]) -> ParsedHeaders:
        ...
        for received in msg.get_all("Received", []):
            if len(received_headers) >= 20:  # Limit to prevent abuse
                break
            received_headers.append(self._clean_header_value(received)[:1000])  # Truncate long headers
    ...
    def _extract_body(self, msg: email.message.EmailMessage, warnings: List[str]) -> Tuple[str, str, str]:
    ...
    def _extract_urls(self, text_body: str, html_body: str, 
                     headers: ParsedHeaders, warnings: List[str]) -> List[ParsedURL]:
