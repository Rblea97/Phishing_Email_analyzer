...
URGENT_PATTERNS = [
    ...
    r"\bsuspend(?:ed)?\s+account\b",
    r"\bact\s+now\b",
    r"\btime\s+sensitive\b",
    r"\blimited\s+time\b",
    r"\b24\s+hours?\b",
    r"\bexpir(?:e|ing)\s+soon\b",
]
...
class ReplyToMismatchRule(Rule):
    ...
    def __init__(self):
        super().__init__("REPLYTO_MISMATCH", "Reply-To domain differs from From domain", 10)
...
class UrgentLanguageRule(Rule):
    ...
    def __init__(self):
        super().__init__("URGENT_LANGUAGE", "Contains urgent or pressure language", 10)

    def check(self, parsed_email: ParsedEmail) -> Optional[RuleEvidence]:
        # Combine subject and body text for analysis
        text_to_check = (
            f"{parsed_email.headers.subject} {parsed_email.text_body} {parsed_email.html_as_text}"
        )
...
class URLShortenerRule(Rule):
    ...
    def __init__(self):
        super().__init__("URL_SHORTENER", "Contains URLs from shortening services", 10)
...
class SuspiciousTLDRule(Rule):
    ...
    def __init__(self):
        super().__init__("SUSPICIOUS_TLDS", "Contains URLs with suspicious TLDs", 10)
...
class UnicodeSpoofRule(Rule):
    ...
    def __init__(self):
        super().__init__("UNICODE_SPOOF", "Potential Unicode spoofing in domains", 10)
...
class NoPersonalizationRule(Rule):
    ...
    def __init__(self):
        super().__init__("NO_PERSONALIZATION", "Uses generic greetings without personalization", 5)
...
class AttachmentKeywordsRule(Rule):
    ...
    def __init__(self):
        super().__init__(
            "ATTACHMENT_KEYWORDS", "Mentions attachments/payments with links present", 5
        )
...
class RuleEngine:
    ...
    def analyze_email(self, parsed_email: ParsedEmail) -> DetectionResult:
        ...
        logger.info(
            f"Email analysis complete: score={final_score}, label={label}, "
