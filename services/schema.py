        ...
        try:
            ...
            if evidence:
                ...
                if total_weight > score * 2:  # Reasonable upper bound
                    return (
                        False,
                        f"Evidence weights ({total_weight}) too high for score ({score})",
                    )

            logger.info(
                f"AI response validation passed: score={score}, label={label}, evidence_count={len(evidence)}"
            )
            return True, None
        ...
        try:
            ...
            for item in evidence[:20]:  # Max 20 items
                if isinstance(item, dict):
                    ...
                    sanitized_item["id"] = "".join(
                        c if c.isalnum() or c == "_" else "_" for c in sanitized_item["id"]
                    )
...
if __name__ == "__main__":
    ...
    valid_response = {
        ...
        "evidence": [
            ...
            {
                "id": "SUSPICIOUS_URL",
                "description": "Contains suspicious .xyz domain",
                "weight": 20,
            },
        ],
    }
